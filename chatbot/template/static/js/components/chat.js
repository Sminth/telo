...
function getBotResponse(text) {
    botResponse = `<img class="botAvatar" src="./static/img/tello.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse;
}
...
function setBotResponse(response) {
    ...
    setTimeout(() => {
        ...
        if (response.length < 1) { 
            ...
            const fallbackMsg = "Oups j'ai un souci, il se peut que le server qui est mon cerveau n'est pas été demarrer 😴😴😴!";

            const BotResponse = `<img class="botAvatar" src="./static/img/tello.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
                ...
                if (Object.hasOwnProperty.call(response[i], "text")) {
                    if (response[i].text != null) {
                        ...
                        if (html.includes("<ul") || html.includes("<ol") || html.includes("<li") || html.includes('<h3')) {
                            html = html.replaceAll("<br>", "");
                            // botResponse = `<img class="botAvatar" src="./static/img/tello.png"/><span class="botMsg">${html}</span><div class="clearfix"></div>`;
                            botResponse = getBotResponse(html);
                        }
                        else {
                            // if no markdown formatting found, render the text as it is.
                            if (!botResponse) {
                                botResponse = `<img class="botAvatar" src="./static/img/tello.png"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
                            }
                        }
